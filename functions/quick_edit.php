...
function custom_quick_edit( $column_name, $post_type ) {
  ...
  if ( ! $dp_options ) $dp_options = get_desing_plus_option();
    ...
    // seo
    if ( in_array( $post_type, array( 'post', 'page', $dp_options['news_slug'], $dp_options['introduce_slug'] ) ) ) {
...
<?php
    ...
    // 紹介
    if ( $post_type == $dp_options['introduce_slug'] ) {
      echo '<input type="hidden" name="introduce_front_meta_box_nonce" value="', wp_create_nonce( 'introduce_cf2.php' ), '" />';
?>
<fieldset class="inline-edit-col-right">
  <div class="inline-edit-col column-recommend">
    <div class="inline-edit-group">
      <label><input type="checkbox" name="show_front_page" value="1" /> <?php _e( 'Display on the front page', 'tcd-w' ); ?></label>
    </div>
  </div>
</fieldset>
<?php
    ...
    // view count
    if ( in_array( $post_type, array( 'post', 'page', $dp_options['news_slug'], $dp_options['introduce_slug'] ) ) ) {
      echo '<input type="hidden" name="view_count_meta_box_nonce" value="', wp_create_nonce( 'view_count.php' ), '" />';
?>
<fieldset class="inline-edit-col-right">
  <div class="inline-edit-col column-view_count">
    <div class="inline-edit-group">
      <label class="inline-edit-view_count" style="float:left;margin-right:1em;">
        <span class="title"><?php _e('View count', 'tcd-w'); ?></span>
        <span class="input-text-wrap"><input type="number" name="_view_count" value="<?php echo intval( get_post_meta( $post->ID, '_view_count', true ) ); ?>" readonly="readonly" style="width:6em;" />
      </label>
      <label class="inline-edit-view_count" style="padding-top:0.2em;">
        <input type="checkbox" name="edit_view_count" value="1" /> <?php _e( 'Edit view count', 'tcd-w' ); ?>
      </label>
    </div>
  </div>
</fieldset>
...
jQuery(document).ready(function($){
    ...
    if ($post_id > 0) {
      ...
      $post_row.find('.custom_quick_edit_values > div > div').each(function(){
        ...
        if (key == '_view_count') {
          ...
          $edit_row.find('[name="edit_view_count"]').removeAttr('checked');
        }
      });
    }
  };
  ...
  $(':checkbox[name="edit_view_count"]').change(function(){
    if (this.checked) {
      $(this).closest('.column-view_count').find('input[name="_view_count"]').removeAttr('readonly');
    } else {
      $(this).closest('.column-view_count').find('input[name="_view_count"]').attr('readonly', 'readonly');
    }
  });
...
function custom_quick_edit_values( $actions, $post ) {
	...
	// seo
	if ( in_array( $post->post_type, array( 'post', 'page', $dp_options['news_slug'], $dp_options['introduce_slug'] ) ) ) {
		$meta_keys[] = 'tcd-w_meta_title';
		$meta_keys[] = 'tcd-w_meta_description';
	}

	// recommend
	if ( $post->post_type == 'post' ) {
		$meta_keys[] = 'recommend_post';
		$meta_keys[] = 'recommend_post2';
	}

	// 紹介
	if ( $post->post_type == $dp_options['introduce_slug'] ) {
		$meta_keys[] = 'show_front_page';
	}

	// view count
	if ( in_array( $post->post_type, array( 'post', 'page', $dp_options['news_slug'], $dp_options['introduce_slug'] ) ) ) {
		$meta_keys[] = '_view_count';
	}
